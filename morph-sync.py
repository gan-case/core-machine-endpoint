...
def morph_images(image_to_morph, f0_location):
    ...
    while(current_gen != total_gen):
        ...
        for file in I_list:
            ...
            json_response = json.loads(requests.post(upload_url,files=upload_data,data=upload_flags).text)['guid']
        ...
        for file1 in range(len(morph_file_input)):
            for file2 in range(file1+1,len(morph_file_input)):
                ...
                morph_response = requests.get(morph_url,params=morph_flags)
...
if __name__ == '__main__':
    morph_images("experiments/test-exp-male-ga-only/raw_images/preprocessed_uploaded_image.jpeg","experiments/test-exp-male-ga-only/SAM_outputs/30/F0/")